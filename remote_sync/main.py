...
def args():
    ...
    parser.add_argument('-p', '--remotePath', help='Path to the folder on remote server', required=True)
...
if __name__ == '__main__':
    ...
    print(f'rsync -rv -e "ssh -p {runtime_arg.port}" {os.path.join(cache_path, tmp_dir, tmp_repo_path)} {runtime_arg.ssh}:{runtime_arg.remotePath}')
    input('== continue? ==')
    os.system(f'rsync -rv -e "ssh -p {runtime_arg.port}" {os.path.join(cache_path, tmp_dir, tmp_repo_path)} {runtime_arg.ssh}:{runtime_arg.remotePath}')